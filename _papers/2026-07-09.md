...
## 4. [MuScriptor: An Open Model for Multi-Instrument Music Transcription](https://arxiv.org/abs/2607.08168v1)

**Authors:** Simon Rouard, Michael Krause, Axel Roebel, Carl-Johann Simon-Gabriel, Alexandre Défossez

**Categories:** `cs.SD, cs.LG`
**Score: 6.90/10** (Obj:9 Id:7 Ind:6 Comp:6 Eff:8 Nov:6 Rep:6)
- **Strength:** 中 — 三阶段训练范式清晰有效，但各组件均为已有方法，GRPO 迁移到 AMT 是增量创新。
- **Weakness:** (1) RL 后训练的 reward（Fonset + Fframe + Foffset）与最终评测指标高度重叠。虽然 DRL（300首）和 DTest（372首）是不同数据集，但 RL 阶段直接优化了评测指标本身，存在 metric hacking 风险——模型可能学会在 reward 计算的容忍窗口内"投机"而非真正提升转录质量。论文未讨论这一风险，也未在独立指标上验证 RL 的效果是否泛化。
...
## 5. [A Quantized Native Runtime for On-Device Semantic Audio Generation](https://arxiv.org/abs/2607.08526v1)
...
## 6. [When Synthetic Speech Is All You Have: Better Call GRPO](https://arxiv.org/abs/2607.08409v1)
...
## 7. [PS4: Proxy-Supervised Joint Training for Real Target Speaker Extraction](https://arxiv.org/abs/2607.08111v1)
...
## 8. [On the Role of Conversational Timing in Synthetic Training Data for ASR](https://arxiv.org/abs/2607.08371v1)
...
## 9. [VSRo-200: A Romanian Visual Speech Recognition Dataset for Studying Supervision and Multimodal Robustness](https://arxiv.org/abs/2607.08112v1)
...
## 11. [COALA: Robust Contextualized Speech-augmented Language Modeling for ASR via Contrastive Regularizer and Biasing Score Estimation](https://arxiv.org/abs/2607.08117v1)

**Authors:** Jhih-Rong Guo, Bi-Cheng Yan, Tien-Hong Lo, Berlin Chen

**Categories:** `cs.CL`
  | Accepted at INTERSPEECH 2026
**Score: 5.70/10** (Obj:8 Id:5 Ind:8 Comp:5 Eff:5 Nov:4 Rep:8)
- **Strength:** 低-中 — DPD-Loss 等价于标准 BCE，MPD-Loss 是 multi-positive contrastive 的标准变体，架构继承自 prior work [13]，新增的方法设计有限。
- **Weakness:** 组件来源分析：(1) Discriminative scoring 架构——直接继承自 prior work [13] (Huang & Wang, ASRU 2025)，包括 frozen backbone + LoRA + discriminative projector 的设计；(2) MPD-Loss——multi-positive contrastive loss 的标准变体，在对比学习文献中已有充分先例；(3) DPD-Lo。
