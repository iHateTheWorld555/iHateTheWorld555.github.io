...
> **中文摘要：** 用户要求我写一段中文摘要,概括这篇论文。英文摘要被截断了,但已经包含了核心信息。我直接根据已有信息写3-5句中文摘要,只输出正文,不加前缀或标题。文本到语音(TTS)系统的评估长期以"自然度"为主要目标,但随着语音保真度的提升,研究焦点已转向"恰当性",即语音是否符合其应用场景。本文探究当下游使用场景变化时,用户感知如何随之改变,针对AI助手、朗读、表演、动画角色和自发对话五种场景,测量了五种主流TTS系统在恰当性与拟人度上的表现。研究发现,恰当性会随领域不同而显著变化,表明自然度并非 universally 适用,TTS评估应结合具体应用场景考察恰当性。
...
> **中文摘要：** 我来获取一下论文的完整摘要，因为目前提供的摘要似乎被截断了。
