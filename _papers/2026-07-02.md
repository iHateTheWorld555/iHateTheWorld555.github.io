...
**Score: 6.95/10** (Obj:9 Id:6 Ind:6 Comp:8 Eff:6 Nov:6 Rep:9)
- **Strength:** 中-高 — 30k 小时 + 0 instruction tuning 达到 SOTA-level，是相对百万小时级 baseline 的 100× 数据压缩。但 emotion understanding 显著输 SOTA，且 speech understanding 需要 prompt 工程，方法效用不全面。
- **Weakness:** 最简 baseline 存在并参与比较：ASR + Text LLM（topline，无遗忘）、Continuous Pre-train、Continuous Pre-train + SFT 三条 Group A 路径都基于同 QWEN3-8B base / 同 30k 小时数据，是干净的对照。这隔离了"是否做权重组合"这一关键变量。
...
## 2. [RT-Tango: Real-Time Distributed Binaural Speech Enhancement for Low-Power Hearing Aid Devices](https://arxiv.org/abs/2607.01834v1)
...
**Score: 6.74/10** (Obj:9 Id:6 Ind:9 Comp:5 Eff:8 Nov:5 Rep:6)
- **Strength:** 中 — 把分布式双耳 SE 在 8 ms 端到端延迟下压到 33 MMACs/s 是真实工程进步，对助听器部署有直接实用价值；GRNN 在 MN-DNN 上 MN=8 退化、FRS 优于 learned skip 等单变量结论对后续系统设计有参考价值。
- **Weakness:** 方法压缩了 Tango 的计算成本（605→33 MMACs/s，约 18×），这是真实的工程压缩，且每个模块（ERB、GRNN、FRS、非对称 STFT、在线 SCM）都有明确数学动机和复杂度推导（GRNN 的 O(H²/G) 推导尤其清晰）。
...
## 3. [SelectTSL: Prompt-Guided Selective Target Sound Localization in Complex Scenarios](https://arxiv.org/abs/2607.02343v1)
...
## 4. [Decomposer: Learning to Decompile Symbolic Music to Programs](https://arxiv.org/abs/2607.01849v1)
...
**Score: 6.53/10** (Obj:9 Id:6 Ind:5 Comp:8 Eff:8 Nov:6 Rep:6)
- **Strength:** 中 — SFT+GDPO+双 reward 的两阶段 pipeline 工程完整、ablation 清晰（图 4、图 5、SFT-only/RL-only 表 2），对音乐 DSL 这一低资源场景的 RL 冷启动有实证贡献；但每个组件单独都不新。
- **Weakness:** 训练-评测闭环部分独立，部分循环。faithfulness reward（onset F1 via mireval）和评测指标（Onset/Frame/Inst F1）共享同一度量族——但这是物理客观指标，不构成污染。
...
## 5. [Self-Supervised Test-Time Tuning for Packet Loss Concealment](https://arxiv.org/abs/2607.01823v1)
...
**Score: 6.32/10** (Obj:8 Id:6 Ind:9 Comp:8 Eff:6 Nov:5 Rep:8)
- **Strength:** 中 — 在两种不同音频领域的两个开源 backbone 上进行了清晰的实例化；因果与非因果分解以及保留集选择是一个稳健的工程框架。缺乏针对非 TTT 逐文件自适应基线和消融实验将突发重放与其他重放策略分离开来，是方法上的缺口。
- **Weakness:** 其机制并非新颖。
...
**Categories:** `cs.CL, cs.ET, cs.LG`
  ...
  | Accepted at Interspeech 2026
**Score: 6.32/10** (Obj:8 Id:6 Ind:8 Comp:6 Eff:6 Nov:5 Rep:8)
- **Strength:** 中 — 跨域分类器 + 方向性 audit 的设计简洁、独立、可迁移，但缺最简 baseline 对照与机制解释。
- **Weakness:** 核心 idea "用人类训练的分类器评 TTS 输出"在 NLP/speech probing 文献中已有先例：Belinkov & Glass 2019 (引用 [22]) 综述 probing classifiers，Cormac English et al. 2022 (引用 [24]) probing wav2vec 2.0 phonetic features，作者自己引用了这条线。
...
**Score: 6.16/10** (Obj:8 Id:5 Ind:8 Comp:8 Eff:5 Nov:5 Rep:6)
- **Strength:** 中 — SFI 卷积迁移到 codec quantizer 邻接层是真实增量，参数效率极高 (3 vs 231/通道对)，但迁移主要发生在同团队自有模块内部，新颖性偏 incremental。
- **Weakness:** 论文隔离了关键变量：Naive (TTR-specific 层切换) 与 Proposed (SFI 层) 唯一差异是"邻接 quantizer 的层是 TTR-specific 还是 SFI"，且每个 input-output 通道对 SFI 层可训练参数仅 3 个 vs Naive 的 231 个。Reference (每 TTR 单独训练) 提供了 TTR-specific 性能上限。
...
## 9. [H-SAGE: Holistic Speaker-Aware Guided Experts for MoE-based Multi-Talker ASR](https://arxiv.org/abs/2607.01566v1)
...
## 10. [Cross Domain Few-Shot Class-Incremental Audio Classification Via Adversarial Contrastive Learning](https://arxiv.org/abs/2607.02254v1)
...
**Score: 5.79/10** (Obj:5 Id:6 Ind:8 Comp:5 Eff:6 Nov:5 Rep:9)
- **Strength:** 中 —— 在 6 对 cross-domain 音频数据集上系统验证了 AT+SC 对 FCAC 的提升，ablation 清晰（虽只在 1 对上做），对音频社区有实用参考价值。
- **Weakness:** CD-FCAC 的对象（base 与 incremental session 之间存在 domain shift）在音频分类场景中确实真实存在，可操作化定义清晰（M 个 session、source/target domain、N-way K-shot）。"智能家居中语音/环境/音乐三类跨域增量识别"的动机合理。
...
## 11. [Rethinking Speech-LLM Integration for ASR: Effective Joint Speech-Text Training by Interleaving](https://arxiv.org/abs/2607.01733v1)
...
## 13. [Enhancing Acoustic-to-Articulatory Inversion with Multi-Target Pretraining for Low-Resource Settings](https://arxiv.org/abs/2607.01594v1)

**Authors:** Jesuraj Bandekar, Prasanta Kumar Ghosh

**Categories:** `eess.AS`
**Score: 5.74/10** (Obj:8 Id:5 Ind:8 Comp:5 Eff:5 Nov:4 Rep:8)
- **Strength:** 低-中 — 多任务预训练 + 已有辅助标签的工程组合，无新机制；"消除 SSL 推理开销"的卖点未量化。
- **Weakness:** 多任务/辅助目标预训练是通用成熟技术。
...
## 14. [DRL-CLBA: A Clean Label Backdoor Attack for Speech Classification via DDPG Reinforcement Learning](https://arxiv.org/abs/2607.01729v1)

**Authors:** Yueming Huang, Wenhan Yao, Fen Xiao, Xiarun Chen, Weiping Wen

**Categories:** `cs.AI, cs.SD`
**Score: 5.74/10** (Obj:8 Id:5 Ind:7 Comp:3 Eff:7 Nov:5 Rep:3)
- **Strength:** 低-中 — 已有组件（音频隐写 + feature collision + DDPG + reward shaping）的组合，复杂度增加但收益仅 ~5 点，没有压缩性洞察。
- **Weakness:** 方法由 4 个已有组件拼装：(1) 音频深度隐写术（Ge et al. 2025, ref 38，整段借用其 encoder-decoder + 损失函数 Eq. 2）；(2) Feature collision（CSSBA, Shen et al. 2023）；(3) DDPG（Lillicrap et al. 2015，标准 RL 算法）；(4) Reward shaping（3 项加权距离 + 分类指示符，Eq. 5）。
...
## 15. [A Multi-Branch Hierarchy-Aware Framework for Heterogeneous Audio Classification](https://arxiv.org/abs/2607.01974v1)
...
## 16. [An Efficient vLLM-Based Inference Pipeline for Unified Audio Understanding and Generation](https://arxiv.org/abs/2607.02119v1)

**Authors:** Haoran Wang, Jinchuan Tian, Siddhant Arora, Shinji Watanabe

**Categories:** `eess.AS, cs.AI`
**Score: 5.47/10** (Obj:8 Id:4 Ind:8 Comp:5 Eff:5 Nov:5 Rep:8)
- **Strength:** 中 — Primary-Auxiliary Decomposition 是一个有用的问题重构；CFG Paired Co-Scheduling 是真实方法增量；其余为标准工程组合。
- **Weakness:** Baseline 选择存在结构性问题。论文 baseline 为"sequential PyTorch inference pipeline"（单请求、无连续 batching），这本质上是一个 straw-man：它将 vLLM 的全部工程优势（PagedAttention、continuous batching、prefix caching、FlashAttention-3）与本文贡献混在一起。
...
## 17. [Spatial Speech Perception Systems: A Survey of Sound Source Localization, Directional Enhancement, and Speech Recognition](https://arxiv.org/abs/2607.02296v1)
...
## 18. [LMPAN: A Lightweight Multi-Path Alignment Network for Joint Full-Duplex Acoustic Echo Cancellation and Noise Suppression](https://arxiv.org/abs/2607.02062v1)
...
## 19. [Pmeta-TLA: Backdoor Attacks for Speech Classification Models via Meta-Learning with Timbre Leakage Attack](https://arxiv.org/abs/2607.01702v1)

**Authors:** Yueming Huang, Wenhan Yao, Fen Xiao, Xiarun Chen, Weiping Wen

**Categories:** `cs.CR, cs.AI, cs.SD`
**Score: 5.00/10** (Obj:6 Id:5 Ind:5 Comp:5 Eff:6 Nov:4 Rep:4)
- **Strength:** 中低 — TLA 是 VSVC 的局部 reframing；Pmeta 训练机制是 MAML+PCGrad 的工程组合，无新机制或新解释，PCGrad vs 其他多任务梯度方法的替代未做。
- **Weakness:** 真实增量有两点：(1) frame-level timbre leakage（vs VSVC 全段）——真实但增量小，本质是把 VSVC 的转换粒度从 utterance 降到 frame；(2) meta+PCGrad 多 backdoor 注入训练流程——MAML、PCGrad、multi-task backdoor 均已有，组合非首创（Federated Meta-Learning Backdoor, 2020 已用 meta-l。
...
## 21. [Using embeddings to predict spoken word duration and pitch in Mandarin monosyllabic words](https://arxiv.org/abs/2607.02002v1)
...
## 22. [Speaker head orientation estimation with a single microphone array using phase spectrogram features](https://arxiv.org/abs/2607.02129v1)
