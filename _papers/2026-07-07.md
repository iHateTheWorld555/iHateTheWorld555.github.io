...
## 1. [Escaping the Procrustean Bed: Groupwise Orthogonal Connectors for Audio-Language Models](https://arxiv.org/abs/2607.06014v1)

**Authors:** Ho-Lam Chung, Ke-Han Lu, Yi-Cheng Lin, Guan-Ting Lin, Yiming Chen et al.

**Categories:** `cs.SD`
**Score: 7.40/10** (Obj:9 Id:8 Ind:6 Comp:8 Eff:8 Nov:7 Rep:5)
- **Strength:** 高 — ORCA 以零额外参数和推理成本，在受控比较中实现 +26.4 点多跳推理提升，方法简洁且因果解释清晰。
- **Weakness:** 可复现性存在显著缺口：(1) 论文未提及代码是否开源——free-search 未找到 ORCA 的公开代码仓库。(2) 训练细节较为充分：5 epochs (~250K steps)、AQA5M 数据、Adam、lr=1e-4、cosine annealing、batch size 96、8×A100-80GB，这些可复现。
...
## 2. [Compress the Cache, Not the Speech Embedding: KV Compression for Efficient Speech LLMs](https://arxiv.org/abs/2607.06827v1)
...
## 3. [LEGATO 2: Toward Multimodal Sheet Music Recognition and Understanding](https://arxiv.org/abs/2607.05769v1)
...
**Score: 7.10/10** (Obj:9 Id:6 Ind:8 Comp:6 Eff:8 Nov:6 Rep:8)
- **Strength:** 中 — pipeline 各模块技术成熟，组合合理，但缺少压缩性创新。逐系统处理和文本嵌入转录是真实改进但概念增量有限。
- **Weakness:** 论文进行了消融实验，分别测试了系统分割模块和识别模块的贡献。然而，作为 pipeline 型工作，核心提升的因果归因存在不清晰之处：(1) Legato 2 相比 Legato 1 的提升同时来自架构改变（逐系统处理 vs. 全页处理）、输出空间扩展（嵌入文本）、训练数据扩展、以及 VLM 模型规模增大，论文虽做了消融但未完全隔离这些变量。
...
## 10. [InsideSSL: Understanding Self-Supervised Speech Representations using a Model-Centric Perspective](https://arxiv.org/abs/2607.06392v1)

**Authors:** Samir Sadok, Xavier Alameda-Pineda

**Categories:** `cs.SD`
  | 10 pages - 9 figures - accepted at INTERSPEECH 2026 (long paper track)
**Score: 6.47/10** (Obj:8 Id:5 Ind:8 Comp:6 Eff:6 Nov:6 Rep:8)
- **Strength:** 中 — GCM 是真正新的诊断工具，per-layer 框架是跨领域迁移。框架统一有价值但各组件非新创。
- **Weakness:** 论文观察到不同训练目标导致不同的压缩/几何/鲁棒性模式（如 Wav2Vec2 的 entropy collapse vs HuBERT/WavLM 的稳定高熵），并将差异归因于训练目标。但论文未隔离关键变量：Wav2Vec2、HuBERT、WavLM、Data2Vec 之间不仅训练目标不同，训练数据量、模型规模、架构细节也有差异。
...
## 11. [Precise Video-to-Audio Generation with Cross-Modal Alignment in Latent Space](https://arxiv.org/abs/2607.06405v1)
...
## 12. [Goodbye Equal Error Rate, Hello Local Information Disclosure: Evaluating Voice Anonymisation against 1-to-N Linkage Threats](https://arxiv.org/abs/2607.06259v1)

**Authors:** Dāvis Šterns, Konstantinos Drossos, Natasha Fernandes, Tom Bäckström, Catuscia Palamidessi

**Categories:** `eess.AS`
**Score: 6.30/10** (Obj:9 Id:6 Ind:8 Comp:8 Eff:6 Nov:6 Rep:5)
- **Strength:** 中 — PMI 在此场景的应用是合理的 reframing，但各组件均为标准技术，新颖性有限。与 SRD 的增量改进（rank → posterior）有意义但非突破性。
- **Weakness:** 论文提供了足够的数学细节（所有公式、校准参数 Table 3、评估系统列表），理论上可复现。但：(1) 未提及代码开源。(2) VPC 2024 的 anonymised datasets 需要从 Challenge organizers 获取，不是公开下载。(3) ECAPA-TDNN 模型权重来自 EURECOM 实验室，论文未说明是否公开。(4) 评估脚本（calibration pipeline、LID 计算）未提供。
...
**Score: 6.10/10** (Obj:8 Id:6 Ind:8 Comp:5 Eff:6 Nov:4 Rep:8)
- **Strength:** 无新方法、新架构或新训练技术提出。所有组件均为已有方法的直接组合应用。
- **Weakness:** 本文的新颖性几乎完全来自"新语言对"（Sinhala→Dhivehi），而非方法或洞察层面的创新。
...
**Score: 5.95/10** (Obj:8 Id:5 Ind:8 Comp:4 Eff:6 Nov:4 Rep:9)
- **Strength:** 中 — 工程管线设计合理，ASR→NMT→多语言BERT→CCMT→KD 的端到端流程完整且可复现，推理时零开销的实用价值是真实的。
- **Weakness:** 方法是已有组件的直接组合：ASR（Faster-Whisper）→ NMT（NLLB-200）→ 语言特定编码器（RoBERTa/RoBERTuito/GBERT/CamemBERT）→ CCMT（直接来自 Ristea et al. 2024，同一实验室同一作者群的先前工作）→ KD/LUPI（Hinton 2015, Vapnik 2015, Lopez-Paz 2016）。
...
## 15. [Determinantal point process sampling for bioacoustic active learning](https://arxiv.org/abs/2607.06063v1)

**Authors:** Hugo Magaldi, Gabriel Dubus

**Categories:** `cs.SD, cs.LG`
  | BioDCASE Challenge 2026 - Task 4 Active learning. Ranked 2/14
**Score: 5.90/10** (Obj:8 Id:5 Ind:8 Comp:5 Eff:6 Nov:5 Rep:6)
- **Strength:** 低-中 — 方法是多组件工程组合，各组件来源可追溯，核心 DPP-for-batch-AL 非原创，新颖性偏增量。
- **Weakness:** 论文提供了消融实验（Table 3），逐一移除 DPP、退火、类别平衡、自适应批次、探索调度，并识别 DPP 批次选择为最大贡献者（移除后 AULC 从 0.50 降至 0.46）。但存在以下缺口：(1) "No DPP batch selection" 消融未说明替换方案是什么——是随机选择 top-q 还是用其他多样性启发式？这影响对 DPP 必要性的判断。
...
## 17. [BlueMagpie-TTS: A Token-Efficient Tokenizer, Language Model, and TTS for Taiwanese-Accent Code-Switching Speech](https://arxiv.org/abs/2607.06054v1)

**Authors:** Ho Lam Chung, Bo-Xuan Zheng, Cheng-Chieh Huang, Cheng-Han Chang, Jung-Ching Chen et al.

**Categories:** `cs.SD, cs.CL`
**Score: 5.58/10** (Obj:8 Id:8 Ind:5 Comp:5 Eff:6 Nov:3 Rep:5)
- **Strength:** 所有组件均为标准技术的直接应用。没有提出新的算法、架构或训练方法。
- **Weakness:** 缺失 BreezyVoice 基线。 BreezyVoice (arXiv:2501.17790, 2025年1月) 是最直接相关的对比对象——同针对台湾普通话 TTS 并拥有开源代码。未将其纳入对比使得效用声明不够完整。
...
## 18. [Andha-Dhun: A First Look at Audio Descriptions in Hindi](https://arxiv.org/abs/2607.06457v1)
...
**Score: 5.30/10** (Obj:8 Id:5 Ind:6 Comp:5 Eff:5 Nov:5 Rep:6)
- **Strength:** 中 — 两阶段 pipeline 设计合理且有效，但各组件均为已有方法的迁移组合，缺少充分的 ablation 验证组件必要性。
- **Weakness:** 论文存在多个识别公理问题
...
**Score: 5.20/10** (Obj:8 Id:5 Ind:6 Comp:5 Eff:5 Nov:5 Rep:5)
- **Strength:** 中 — 方法正确且物理一致（满足 Helmholtz 方程），源位置条件化是合理改进，但属于已有框架的增量扩展。
- **Weakness:** 论文的核心改进是将源位置 y 作为 INR 的额外输入（公式 5），从单快照优化转为多源训练。SB-NK baseline 使用相同网络架构但移除源依赖组件——这是一个合理的消融实验，隔离了源位置依赖这一关键变量。然而，存在严重缺陷：(1) 只有 3 个 baseline（SB-NK、Uniform、以及 SB-NK 本质上是 LB-NK 的消融版），缺少与其他类别方法的比较。
...
## 22. [TriA Pipeline: A Large-Scale Automatic Audio Annotation Pipeline For Audio Classification In Specific Scenarios](https://arxiv.org/abs/2607.06179v1)
